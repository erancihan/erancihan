...
Chapter 05.A built classic `VkRenderPass` + `VkFramebuffer` objects to make the
...
We create five pipelines cold at startup (chapter 07.B). A **`VkPipelineCache`** lets
...
"pipeline explosion" chapter 07.B flagged: fewer objects, each covering more cases.

### VMA tuning — stop re-uploading everything

Our per-frame instance and HUD buffers are sized to a generous max and fully
re-`memcpy`'d each frame (chapters 06 and 08). Real usage: grow buffers on demand, only
...
Chapter 05.B keyed the `renderFinished` semaphore by frame-in-flight for clarity
and noted the subtlety. The fully-correct form keys it **per swapchain image** (since an
...
Our lighting is one directional term (chapter 07.A). The upgrade path, in order of
bang-for-effort:
...
- **GPU-driven rendering** — combine the instance SSBO with `vkCmdDrawIndexedIndirect`
  ...
  instancing path chapters 07.B and 08 built, and where bindless pays off.
...
- **Validation layers, harder.** You've had the basics on all along (chapters 01–02).
