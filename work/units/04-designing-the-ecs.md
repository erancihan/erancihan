...
- **Component** — a plain-data struct, no methods. A `Transform`, a `Velocity`,
  an `Enemy`. It answers *"what does this entity have?"* (Chapter 07.)
...
By chapter 11 the player ship will be an id holding a `Transform`, `Velocity`,
...
**`Sources/SpaceFighter/ECS/Entity.swift`** — new file (`mkdir -p Sources/SpaceFighter/ECS` first):
...
entities — but the direction you'd grow toward (chapter 12).
...
**`main.swift`** — replace the whole file (throwaway; chapter 06 writes the real
one):
