...
// Chapter 12: the flagship against the ORDER-FAITHFUL GOLDEN CHAIN --
// tb_add4_stream's harness with one line changed (the oracle instance)
// and a fresh seed, so the two proofs share no stimulus. The reference
// is ch10's ref_add4_tree: three ch08 fp32_add_alg golden adders wired
// in the tree's exact association (a+b)+(c+d). Full-32-bit result
// compare (NaN payloads included) plus all three flags -- this is the
// only shipped bench anywhere in the repo that checks the pipelined
// flagship against the golden chain directly, discharging spec S9's
// second streaming obligation with an oracle that shares no RTL with
// the DUT's datapath modules.
//
// Ships at NPR = 12000 (~61k valid quads, 36 s here) for the same
// timeout-margin reasons as tb_add4_stream; the 101k headline run is
// the same source at -DNPR=20000, out of harness.
`ifndef NPR
 `define NPR 12000
`endif

module tb_add4_gold;
  localparam [31:0] SEED = 32'd777214;
  ...
  ref_add4_tree refc (.a(a), .b(b), .c(c), .d(d), .result(r_c),
  ...
  initial begin : watchdog
    #60_000_000;
    $display("FAIL tb_add4_gold: timeout");
    $fatal(1, "timeout");
  end
      ...
      if (cyc >= LAT) begin
        if (vq[idx]) begin
          if (ov !== 1'b1) begin
            ...
            if (mismatches <= 10)
              $display("FAIL tb_add4_gold cyc %0d: retired out_valid=%b, expected 1",
                       cyc, ov);
          end else if ((^{r_p, inv_p, ovf_p, inx_p}) === 1'bx) begin
            xfails = xfails + 1; mismatches = mismatches + 1;
            if (mismatches <= 10)
              $display("FAIL tb_add4_gold cyc %0d: X in outputs: %h/%b%b%b",
            ...
            if (mismatches <= 10)
              $display("FAIL tb_add4_gold cyc %0d: pipe %h/%b%b%b tree %h/%b%b%b",
                       cyc, r_p, inv_p, ovf_p, inx_p,
          ...
          if (ov !== 1'b0) begin
            mismatches = mismatches + 1;
            if (mismatches <= 10)
              $display("FAIL tb_add4_gold cyc %0d: bubble retired out_valid=%b",
                       cyc, ov);
          end
        ...
        if (ov !== 1'b0) begin
          mismatches = mismatches + 1;
          $display("FAIL tb_add4_gold cyc %0d: out_valid=%b during fill", cyc, ov);
        end
      end
      in_valid = valid;
      a = wa; b = wb; c = wc; d = wd;
  ...
  initial begin
    ...
    if (n_valid_driven !== 16) begin
      $display("FAIL tb_add4_gold: directed phase drove %0d quads, expected 16",
               n_valid_driven);
    ...
    repeat (3) begin
      ...
      if (ov !== 1'b0) begin
        mismatches = mismatches + 1;
        $display("FAIL tb_add4_gold: out_valid=%b after drain", ov);
      end
    end

    // --- count guards ---
    if (n_checked_valid !== n_valid_driven) begin
      $display("FAIL tb_add4_gold: %0d valid quads driven but %0d checked (truncated drain)",
               n_valid_driven, n_checked_valid);
      $fatal(1, "drain count");
    end
    if (ov_cycles !== n_valid_driven) begin
      $display("FAIL tb_add4_gold: %0d out_valid cycles for %0d valid quads",
               ov_cycles, n_valid_driven);
      $fatal(1, "throughput count");
    end
    if (n_valid_driven !== 16 + 5*NPR + mix_valid) begin
      $display("FAIL tb_add4_gold: drove %0d valid quads, expected %0d",
               n_valid_driven, 16 + 5*NPR + mix_valid);
      $fatal(1, "drive count");
    end
    if (n_valid_driven < 60017) begin
      $display("FAIL tb_add4_gold: only %0d valid quads driven (this target needs >= 60017)",
               n_valid_driven);
      $fatal(1, "drive floor");
    end
    if (mismatches !== 0) begin
      ...
      $fatal(1, "FAIL tb_add4_gold: %0d mismatches", mismatches);
    end
    $display("PASS tb_add4_gold (%0d valid quads incl 16 directed, %0d bubbles, 1 result/cycle, latency 4)",
