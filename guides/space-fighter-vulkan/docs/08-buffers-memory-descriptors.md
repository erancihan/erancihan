# 08 · Buffers, memory (VMA) & descriptors 🛠️
...
> uniforms and instancing of chapters 04 and 07 become real memory.
...
match on both sides *exactly*. These are the render types (`render/types/GPUContract.hpp`)
and their GLSL counterparts from chapter 07:

| C++ (`render/types/`) | GLSL | Bytes | Where it's used |
|---|---|---|---|
...
- **Vertex buffers** are laid out by *your* binding stride and attribute offsets
  (chapter 07). `Vertex` packs tight at 24 bytes because we declared
...
The allocator is created next to the device, so it lives in
`render/rhi/Context.cpp` with the rest of chapter 03's setup. Everything below is
`render/rhi/Buffers.cpp`.
...
The ship, enemy, bolt, star and grid meshes each go through this once, at startup,
inside the **mesh registry** — `render/MeshRegistry.cpp`, chapter 09. That's the
only caller `uploadStatic` has, which is the point of the split: `rhi/Buffers`
knows how to move bytes to the GPU and nothing about what the bytes are, and
`MeshRegistry` knows what art exists and nothing about staging.
...
Crucially, these are allocated **per frame-in-flight** (chapter 06). While the GPU
...
Chapter 06 deferred the depth image to here, because it needs VMA. It's the same
`createBuffer` idea for an *image* — device-local, no CPU access — and it lives in
`render/rhi/Swapchain.cpp` rather than `Buffers.cpp`, because it's sized to the
swapchain extent and is destroyed and rebuilt with it on every resize:
...
// then a VkImageView with aspect = VK_IMAGE_ASPECT_DEPTH_BIT (chapter 06's framebuffers use it)
```
...
this in three layers, which is the part that feels heavy the first time. All three
live in `render/rhi/Descriptors.cpp`:
...
This layout also feeds the **pipeline layout** (chapter 07), so the pipeline knows
...
We use one for the star field's tiling `span` (chapter 09), passed straight to the
star pipeline:
...
Here's how chapter 07's instancing actually gets fed. Each frame, `RenderSystem`
...
void Renderer::drawGroup(VkCommandBuffer cmd, const Group& g) {
    const GPUMesh& m = meshes.get(g.mesh);                         // the MeshRegistry, ch 09
...
That's `drawGroup`, and chapter 07's `drawLit` and `drawGlow` are each a
`vkCmdBindPipeline` and a filtered loop over it.
...
a plain non-indexed `vkCmdDraw` (they carry no index buffer, chapter 09). The grid
contributes one hand-built `InstanceData` — a model matrix snapped to the nearest
whole cell around `focus`, chapter 09 — appended after the entity groups; the
stars need none at all, since `star.vert` positions them from the camera alone.
The registry keys them by a separate `SceneryID` for exactly this reason: they are
art the *renderer* schedules, not art an entity asks for, and giving them their
own enum keeps `MeshID` meaning "a mesh some entity can wear."
...
> and avoid re-uploading unchanged data — chapter 15.
...
[Chapter 09: Meshes & simple geometry](09-meshes-and-geometry.md)
