...
> [Metal guide](../../space-fighter-metal/docs/10-flight-and-input.md); only the
...
struct InputState {
    ...
    float throttle = 0;   // wired through, unused — chapter 15's first exercise
    bool  firing   = false;
    bool  boosting = false;
};
```
...
> `glfwPollEvents` in the game loop (chapter 10) is what refreshes both.
...
This is chapter 04's local-space rotation, made concrete. Each frame we build a
...
- **Rates × dt.** `pitchRate` is ~1.7 rad/s; multiplying by `dt` makes the turn
  frame-rate independent (chapter 10).
...
world space (chapter 04). Multiply by speed and that's the velocity the
...
inherited — chapter 05's promise, cashed out.

---

**Next:** now that the ship moves, we need a camera that follows it well. →
[Chapter 12: The camera](12-the-camera.md)
