...
> `Entity`, `ComponentStore` and `World` — the three headers in `ecs/` — in C++. This chapter is the one place the
...
- **Component** — a plain-data struct, no methods. A `Transform`, a `Velocity`,
  an `Enemy`. It answers *"what does this entity have?"* Ours live in
  `components/`, four headers split by role:

  | Header | Holds | Written in |
  |---|---|---|
  | `components/Spatial.hpp` | `Transform`, `Velocity` | chapter 10 |
  | `components/Rendering.hpp` | `Renderable` | chapter 10 |
  | `components/Physics.hpp` | `Collider`, `Layer` | chapter 13 |
  | `components/Gameplay.hpp` | `Player`, `Weapon`, `Enemy`, `Projectile`, `Lifetime`, `Homing`, `Spinner` | chapters 11–13 |

  One `Components.hpp` would work — they're all POD structs and the file would be
  eighty lines. Four is better for a C++-specific reason: a header is a
  recompilation unit. `MovementSystem.cpp` needs `Transform` and `Velocity` and
  nothing else, so it includes `Spatial.hpp` and stays untouched when you add a
  `Shield` to `Gameplay.hpp`. The split also makes each system's include list a
  readable statement of what it actually reads — which is the same property
  chapter 02's boundary test leans on.
...
component *types* and zero code.

Neither recipe lives in `Game.cpp`. The list of components that makes a thing a
player ship is its own kind of fact — Unreal keeps it in a Blueprint asset, and we
keep it in `archetypes/Player.cpp` as `spawnPlayer(World&)` (chapter 02 explains
why; chapter 10 writes it). `Game.cpp` owns *when* things happen, `archetypes/`
owns *what things are made of*, and keeping those apart is what stops both files
from growing a second job.
...
you'd grow toward. Chapter 15 revisits this.
...
onto"), add generations. Chapter 15 shows the shape.
...
1. **Add a component** (pure data) to the header that matches its role — a
   `Shield` is gameplay, so `components/Gameplay.hpp`:
   ```cpp
   struct Shield { float strength; };
   ```
2. **Attach it** in the archetype that should have it — one line in
   `archetypes/Enemy.cpp`: `world.add(e, Shield{3.0f});`.
3. **Write a system** in `systems/ShieldSystem.cpp` that runs over
   `world.store<Shield>().entities()`.
4. **Schedule it** — one line in `Game::update`, in the right spot in the order.
5. **Build it** — one line in `CMakeLists.txt`'s `add_executable` list.

Five files touched, four of them by one line, and not one existing system edited.
That decoupling — data in components, behaviour in systems, recipes in archetypes,
wired in one schedule — is the entire reason to build this way, and it's identical
whether the pixels come out of Metal or Vulkan.
...
[Chapter 06: Swapchain, commands & sync](06-swapchain-and-sync.md)
