...
> HUDs, and where you'd add real text — reading `systems/HUDSystem.cpp` and `Renderer::drawHUD`.
...
```cpp
// render/types/HUDVertex.hpp
struct HUDVertex { glm::vec2 position; glm::vec4 color; };
```

It sits in `render/types/` beside `InstanceData` for the same reason: it's a struct
the GPU's vertex-input layout must match exactly, and it's produced by gameplay
code. `render/types/` is the one directory both sides include, which is why
chapter 02's third boundary check exists to keep any Vulkan header out of it.

(This is the struct the `hud` pipeline's own vec2+vec4 vertex-input layout was
built for back in chapter 07 — the one pipeline that doesn't consume `Vertex`.)

### The Vulkan Y-down twist

Here's the one place Vulkan's downward NDC (chapter 04) leaks into gameplay code.
...
`HUDSystem::build` assembles the whole overlay into one flat `std::vector<HUDVertex>`,
...
**Painter's order + no depth.** The HUD uses the `hud` pipeline (chapter 07), which
...
Like the instance buffer (chapter 08), the HUD buffer is **per frame-in-flight** and
...
- **Bake glyphs to a texture atlas.** Render a font once into a texture, then draw
  ...
  descriptor, which chapter 15's roadmap touches).
...
`HUDSystem::build` takes `GameStats` and an aspect ratio and returns a vector of
...
That's also why it's in `systems/` and not in `render/`, despite being the file
with the most drawing vocabulary in it. What it knows is `GameStats` — health,
score, a flash timer — which is gameplay. What it emits is `HUDVertex`, which is
plain data. Nothing in it says `Vk`, and `Renderer::drawHUD` is a `memcpy`, a bind
and a draw over the result.
