...
> where the *idea* is identical to the [Metal guide](../../space-fighter-metal/docs/11-the-camera.md);
...
the shaders (chapter 04). Producing it is one system that runs late in the frame,
...
> (chapter 04), applied once when we build the projection. Keep them there and the
...
`Math::lookAt` (chapter 04, `glm::lookAtRH`) turns eye/center/up into the view
matrix.
...
- **`aspect`** comes live from the framebuffer size each frame, so resizing the
  window never stretches the image (chapter 04). In Vulkan this pairs with the
  swapchain-recreation path (chapter 06): on resize, the new extent flows into
...
- **near/far = 0.1 / 1200** is the depth range. Too wide a range wastes depth
  ...
  0..1 depth range, from chapter 04, already uses that precision better than
  OpenGL's −1..1 would.)
...
[Chapter 13: Gameplay systems](13-gameplay-systems.md)
