...
one GLFW call, `glfwCreateWindowSurface`, shown in chapter 06 — because picking a
...
APIs keep inside. Our `VulkanContext` (`render/rhi/Context.hpp`, and the only place in the
project that says `VkInstance`) owns the
...
features.largePoints = VK_TRUE;                  // stars draw as >1-px points (chapter 09)
...
When you eventually call `vkCmdDrawIndexed` (chapter 07), the GPU runs a fixed
...
- **Vertex shader** — runs once per vertex. Its job: place the vertex in *clip
  space* by multiplying through the model and camera matrices (chapter 04). Ours
  also passes the world normal and color along (`lit.vert`, chapter 07).
...
until *you* create it — chapters 06 and 08 allocate it by hand (MetalKit's view
managed one for us in the sibling guide). Second, the test policy — compare how?
write or not? — is frozen per **pipeline** (chapter 07), and choosing it per pass
...
You now have a configured GPU and nowhere to draw. Chapter 06 builds the
...
NDC. → [Chapter 04: The math you need](04-the-math-you-need.md)
