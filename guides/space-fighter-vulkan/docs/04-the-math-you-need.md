...
> line-by-line grip on `core/Math.hpp`, and — the Vulkan-specific part — the **three
...
retune it for Vulkan — put these at the top of `core/Math.hpp`:
...
exactly what projection expects. Chapter 12 uses it. Note the view matrix needs
...
`FlightControlSystem`, chapter 11).
...
- Our `FrameUniforms` uses `vec4` for `cameraPosition` and `lightDirection` for
  exactly this reason (chapter 08 lays the structs out in full and revisits this).
...
[Chapter 05: Designing the ECS](05-designing-the-ecs.md)
