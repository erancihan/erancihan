...
> timing ideas are identical to the [Metal guide](../../space-fighter-metal/docs/09-the-game-loop.md);
...
while (!glfwWindowShouldClose(window)) {
    ...
    renderer.drawFrame(data);                                   // draw (chapter 06)
}
...
**swapchain's present mode** (chapter 06). We chose `FIFO`, which is vsync: when
...
Vulkan, where a `recreateSwapchain` on resize (chapter 06) can produce exactly the
...
None of those apply to us yet, so we keep the simpler model. Chapter 15 flags the
...
```cpp
// Game.cpp — one line per file in systems/
...
- `flushDestroyed` runs **last and once**, so every system saw a consistent world
  all frame and deletion happens at a single safe point (chapter 05) — which in
...
Two things are deliberately *not* in that list, and knowing why is knowing the
layout:

- **Spawning.** `spawnPlayer`, `spawnEnemy` and `fireBolt` are recipes, not steps —
  they say what a thing is made of, not when anything happens. They live in
  `archetypes/` (chapter 02), and the systems that need one call it. Put them here
  and `Game.cpp` grows a second job.
- **Collecting what to draw.** After `flushDestroyed`, `RenderSystem` sweeps every
  entity with a `Transform` and a `Renderable` and buckets them by mesh, and
  `HUDSystem` builds the overlay's vertices. Both return plain data into
  `FrameData`. They're the last thing the *simulation* does, not the first thing the
  *renderer* does — which is exactly why the renderer can be handed a struct and
  never see the world it came from.
...
[Chapter 11: Flight & input](11-flight-and-input.md)
