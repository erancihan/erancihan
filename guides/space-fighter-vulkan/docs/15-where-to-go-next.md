...
> [Metal guide](../../space-fighter-metal/docs/15-where-to-go-next.md); the
...
1. **A throttle.** `Player.throttle` and `InputState.throttle` are already wired and
   unused (chapter 11). Map `Left Shift`/`Left Ctrl` to ease `throttle`, and use it
   to lerp speed between a min and max. *Seam: input → flight.*
2. **A new enemy shape + behaviour.** Add a `MeshID` value and classify it in
   `materialOf`, write a mesh generator in `content/meshes/` (chapter 09), add a
   component in `components/Gameplay.hpp` and a system for a new movement pattern (a
   strafer that circles you), and give it an archetype. Count the files you touched
   and note that `render/` isn't among them. *Seam: content + component + schedule.*
3. **An explosion.** Write `spawnDebris` in `archetypes/`, and call it from
   `CollisionSystem` on a kill: a handful of short-lived entities with outward
   velocities and a `Lifetime` — instant particle burst with the tools you already
   have. *Seam: gameplay → ECS.*
...
Chapter 06 built classic `VkRenderPass` + `VkFramebuffer` objects to make the
...
We create five pipelines cold at startup (chapter 07). A **`VkPipelineCache`** lets
...
"pipeline explosion" chapter 07 flagged: fewer objects, each covering more cases.

### VMA tuning — stop re-uploading everything

Our per-frame instance and HUD buffers are sized to a generous max and fully
re-`memcpy`'d each frame (chapters 08 and 14). Real usage: grow buffers on demand, only
...
Chapter 06 keyed the `renderFinished` semaphore by frame-in-flight for clarity and
...
- **glTF 2.0** — the open standard, and the right first target. Use
  ...
  upload with the same `uploadMesh` (chapter 09).
...
physics step — and with it, the **fixed timestep** from chapter 10. Options:
...
- **Broad-phase collision** (chapter 13) — a uniform grid or spatial hash so
...
- **Entity generations** (chapter 05) — fold a generation into the id so long-lived
...
- **GPU-driven rendering** — combine the instance SSBO with `vkCmdDrawIndexedIndirect`
  ...
  instancing path chapter 07 started, and where bindless pays off.
...
- **Android** — Vulkan is a first-class Android API; the renderer ports with a
  ...
  chapter 11 earns its keep).

---

## Going multiplayer

Co-op or versus dogfighting is the big one, and it reaches back into chapter 10's
...
Vulkan, that separation is half-done — the schedule (chapter 10) is the thing you'd
...
Every item above plugs into a seam this guide already built, and the tree is the
map of them: content swaps behind `MeshData` in `render/types/`, behaviour is a new
component + system + archetype + schedule line, rendering upgrades live behind the
instance-SSBO handoff, every `Vk` call in the project is inside `render/rhi/`, and
the frame's sync lives in `Renderer::drawFrame`. The timestep decision gates physics
and netcode. And `tests/BoundaryTest.cpp` is still running, so the seams you're
building on are the seams that are actually there — not the ones the directory
names merely suggest. That's the real deliverable
