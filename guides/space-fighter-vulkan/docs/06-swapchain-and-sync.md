...
once-only objects this chapter builds live in `render/rhi/Context.cpp` and
`render/rhi/Swapchain.cpp` from chapter 01's map; the per-frame sequence at the
end is `Renderer::drawFrame`. Everything in this chapter is `rhi/` — it is the
one directory allowed to name a `Vk` type, and chapter 02's boundary test is the
...
That surface is what chapter 03's device selection tested for "can this queue
...
loop is vsync-paced, which chapter 10 leans on.
...
depth image, back it with memory (VMA — chapter 08), and make a view, exactly once
(and again on resize). Conceptually it's the depth image from chapter 03's
...
We'll write the actual image-creation code in chapter 08 once VMA is in hand; for
...
> because it's what most existing samples show. Chapter 15 points you at dynamic
...
values, (the draws go here, chapters 07–08 and 14), end:
...
the draws inside are chapters 07, 08 and 14. What's genuinely new is what happens
...
reason (chapter 07), so a resize doesn't force a pipeline rebuild.
...
- The **render pass** declares attachments, load/store ops, and layout
  transitions up front; **dynamic rendering** is the modern shortcut (chapter 15).
...
shaders. → [Chapter 07: The graphics pipeline & SPIR-V](07-the-graphics-pipeline.md)
