...
|---|---|
| ⭐ Apple — **Metal** documentation — https://developer.apple.com/documentation/metal | The API reference for every object in Chapter 03: device, command queue, pipeline state, encoders. |
| Apple — **MetalKit** / `MTKView` — https://developer.apple.com/documentation/metalkit | The view + delegate that drive our game loop (Chapters 03, 07). |
| ⭐ Apple — **Metal Shading Language Specification** (PDF) — https://developer.apple.com/metal/Metal-Shading-Language-Specification.pdf | The definitive MSL reference — attributes like `[[vertex_id]]`, `[[instance_id]]`, `[[point_coord]]` used in Chapter 07. |
...
| Apple — **Model I/O** — https://developer.apple.com/documentation/modelio | Loading real 3D assets (USD/OBJ) when code geometry stops being enough (Chapter 15). |
| Apple — **simd** — https://developer.apple.com/documentation/accelerate/simd | The vector/matrix/quaternion types (Chapter 04) that also match Metal's memory layout. |
...
| **Metal Debugger / GPU Capture** (Xcode docs) — https://developer.apple.com/documentation/xcode/metal-debugger | Capture a frame and inspect every draw call and buffer — the tool from Chapter 15. |
...
| Resource | What it's for |
|---|---|
| ⭐ **Ben Eater & Grant Sanderson — Visualizing quaternions** (interactive) — https://eater.net/quaternions | The best intuition for *why* quaternions rotate without gimbal lock (Chapter 04, 10). |
...
| **Song Ho Ahn — OpenGL projection & transformation matrices** — https://www.songho.ca/opengl/gl_projectionmatrix.html | Derivations of the perspective and lookAt matrices in `Core/Math.swift` (mind the [0,1] vs [-1,1] depth note). |
