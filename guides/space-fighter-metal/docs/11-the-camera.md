...
Chapter 09 left three inline lines doing that job in `Game.update`. They work,
...
**World up, `(0,1,0)`** — rock steady, and what chapter 09 used. The camera
...
chapter 04 built in, and it's why we can pass a blended, non-perpendicular vector
here without doing any correction ourselves.

## Update `Game.swift`

**`Game.swift`**, in `update` — replace chapter 09's inline camera with the
system:
...
camera, don't move the near plane. 1200 comfortably contains chapter 06's star
...
invisible, exactly like chapter 09. Now try `* 0.0 + t.up * 1.0`: the entire
...
the `exp` form above, or `Math.moveToward` from chapter 04); and decide where
