...
> show up in every frame — so when you write the renderer in chapter 08 it reads
> like prose, not spells.
>
> **Files created: none.** This is a concept chapter. Read it, then chapters 06
...
first group once in `Renderer.init` (chapter 08) and the second group fresh every
...
are created **fresh every frame**. That split is exactly how chapter 08 is
...
chapters 07 and 08, shown here so the vocabulary has something concrete attached.
...
`ShaderLibrary` is twelve lines we write in chapter 07. It reads the project's
`.metal` files, concatenates them, and calls `makeLibrary(source:)`, which
compiles Metal Shading Language on the spot and hands back the functions by name.
(Bigger projects ship a `.metallib` compiled by their build system instead; we
trade a few milliseconds at launch for a package that builds with `swift run` —
chapter 07 explains why that isn't a free choice here.)
...
- **Vertex shader** — runs once per vertex. Its job: place the vertex in
  *clip space* by multiplying through the model and camera matrices. Ours also
  passes the world normal and colour along. (`lit_vertex` in `Shaders/lit.metal`.)
...
You'll set it up in `main.swift` and `Renderer.init` in chapter 08, then let it
drive you. Chapter 09 picks up that `draw(in:)` callback as the game loop's
heartbeat.
...
You now have the vocabulary. Chapters 07 and 08 spend it on real shaders and a real draw
...
[Chapter 04: The math you need](04-the-math-you-need.md)
