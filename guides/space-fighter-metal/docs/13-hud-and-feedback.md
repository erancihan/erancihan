...
>
> **Files created:** `Sources/SpaceFighter/Systems/HUDSystem.swift`
...
Most of the machinery is already written. The shader in 07 widens a 2D point to
4D and passes the colour through with no transform at all, and `Renderer.drawHUD`
exists and is already being called every frame — it just early-returns because
`Game.update` hands it an empty array.

Both halves of the machinery are already written. `HUDVertex` went into
`Render/Types/HUDVertex.swift` in chapter 08 — a 2D position and a colour, no
normal, no matrix — because `Renderer.render` takes an array of them. The shader
in 07 widens the 2D point to 4D and passes the colour through with no transform
at all.
...
**`Sources/SpaceFighter/Systems/HUDSystem.swift`** — new file:
...
**`HUDSystem.swift`**, in `appendRect` — after the four corners:
...
**`HUDSystem.swift`**, in `HUDSystem` — before `appendRect`:
...
**`HUDSystem.swift`**, in `build` — after the flash block:
...
**`HUDSystem.swift`**, in `build` — after the reticle:
...
That's it. `Renderer.drawHUD` has been waiting since 08.

---

## Why the HUD draws correctly over everything

Two decisions from 08 are what make this work, and they're worth collecting now
...
`HUDSystem.build` takes stats and an aspect ratio and returns an array. No state, no
side effects, and it never touches the renderer — it produces data the renderer
consumes, exactly like `SceneSystem` produces `InstanceData`.
...
Three things to work out. `HUDSystem.build` currently receives only `GameStats`, so it
...
**Next:** stop recompiling the same shaders on every launch. →
[Chapter 14: Caching the compiled shaders](14-shader-caching.md)
