...
>
> **Files created:** `Sources/SpaceFighter/Systems/WeaponSystem.swift`,
> `Systems/EnemySystem.swift`, `Systems/LifetimeSystem.swift`,
> `Systems/CollisionSystem.swift`, `Archetypes/Projectile.swift`,
> `Archetypes/Enemy.swift`
...
`ship.right` is chapter 09's computed property, so "wingtips" is just ±0.9 along
...
That `scale` is chapter 06's cube being restyled: `(0.18, 0.18, 0.7)` stretches
...
The system has now decided *when* to fire and *where the muzzle is*. What a bolt
is made of is the archetype's business:
...
**`Sources/SpaceFighter/Archetypes/Projectile.swift`** — new file:
...
Note where the seam fell. `WeaponSystem` owns the *rules* — cooldowns, muzzle
offsets, which wingtip fires. `Archetypes/Projectile.swift` owns the *recipe* —
damage, radius, colour, how long it lives. Retuning the gun and redesigning the
bullet are different jobs, and now they're different files. It also means a
second thing that fires bolts (a turret in chapter 15, say) reuses the recipe
without inheriting the gun.
...
It's safe for exactly one reason: chapter 05's `destroy` only appends an id to a
...
## Enemies: a director, two flavours, and homing
...
The director has now picked a spot. Handing off to the archetype is one line:
...
**`Sources/SpaceFighter/Archetypes/Enemy.swift`** — new file:
...
Now the two flavours — the same mesh, different components:

**`Archetypes/Enemy.swift`**, in `spawnEnemy` — after `towardPlayer`:
...
One more small helper, used only here:

**`Archetypes/Enemy.swift`** — after `spawnEnemy`:
...
`Spinner`. `SpinSystem` from chapter 09 already tumbles the drifters without
knowing enemies exist, and `steerHomers` below steers the chasers without knowing
about drifters.

The colour difference isn't decoration either — it's the only way the player can
tell which one is about to turn.

Both flavours live in one archetype file rather than two, because they share the
transform, the collider and the health — the difference is three components at
the end. Split them the day the shared part stops being shared. That is the same
judgement `Content/Meshes/SceneryMesh.swift` made about the grid and the
starfield, and it will be wrong eventually in both places; the point is to notice
when.
...
Both of chapter 04's vector products appear here doing exactly what they were
...
`hitFlash = 0.5` is a request the HUD will honour in chapter 13 — collision knows
...
is beats optimising before you reach it. Chapter 15 flags this as the first thing
...
We re-query the transform *after* the systems have run, so `focus` is
...
stays valid — which is exactly why chapter 05 could get away with never
...
- `Weapon.fireInterval` in `Components/Gameplay.swift` 0.14 → 0.05 — a minigun.
...
[Chapter 13: HUD & feedback](13-hud-and-feedback.md)
